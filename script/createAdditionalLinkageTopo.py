...
def generateConfigFile(contentList: list):
    ...
    for i in range(workerNum):
        commonConfig.execCmd(commonConfig.getWorkerName(i), "touch /root/lemethod.conf")
        commonConfig.execCmd(commonConfig.getWorkerName(i), "rm /root/lemethod.conf")
        commonConfig.execCmd(commonConfig.getWorkerName(i), "touch /root/lemethod.conf")
        commonConfig.execCmd(commonConfig.getWorkerName(i),
                             f'bash -c "echo \'{content}\' >> /root/lemethod.conf"')
    commonConfig.execCmd("server", "touch /root/lemethod.conf")
    commonConfig.execCmd("server", "rm /root/lemethod.conf")
    commonConfig.execCmd("server", "touch /root/lemethod.conf")
...
def createAdditionalLinkageTopo() -> None:
    ...
    for i in range(workerNum):
        if ufs.isFather(i) and ufs.count(i) == 1:
...
if __name__ == '__main__':
    ...
    if addtionalLinkageNum < 0 or addtionalLinkageNum > workerNum * (workerNum - 1) / 2:
        ...
        exit(1)
    projectName = "LM" + str(workerNum) + "Additional" + \
        str(addtionalLinkageNum) + "LinkageTopo"
