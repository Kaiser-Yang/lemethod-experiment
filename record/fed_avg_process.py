...
opt_labels = ['多步优化得到 $w_{t+1}^{(1)}$', '多步优化得到 $w_{t+1}^{(2)}$', '多步优化得到 $w_{t+1}^{(3)}$']
nodes = []
for i, (x, y) in enumerate(node_positions):
    ...
    ax.text(x, y - 0.3, opt_labels[i], ha='center', va='center', fontsize=14)
    nodes.append((x, y))
...
# 广播标注
ax.text(-2.9, 3.2, '① 广播 $W_t$, 本地轮数 $K$', fontsize=14, color='blue',
        bbox=dict(boxstyle="round,pad=0.3", facecolor='white', edgecolor='blue'))

# ========== 步骤 2：本地多步优化（已在节点内部标注） ==========
ax.text(-3.8, 2.2, '② 本地多步优化', fontsize=14, color='darkred', fontweight='bold',ha='center', va='center', bbox=dict(boxstyle="round,pad=0.3", facecolor='white', edgecolor='navy'))

# ========== 步骤 3：上传本地模型参数 ==========
# 上传标注
ax.text(0.3, 3.2, '③ 上传 $w_{t+1}^{(k)}$', fontsize=14, color='red',
        bbox=dict(boxstyle="round,pad=0.3", facecolor='white', edgecolor='red'))

# ========== 步骤 4：加权聚合与更新（已在服务器内部显示） ==========
ax.text(ps_x + 1.7, ps_y+0.2, '④ 加权聚合得到 $W_{t+1}$', fontsize=14, color='navy', fontweight='bold',
